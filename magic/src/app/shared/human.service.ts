...
export class HumanService {
  ...
  public login(human: Human){
    if(human.username && human.password){
      ...
      return this.http.post(this.url + "/login/", body, {headers: this.headers, withCredentials: true}).pipe(map( resp => resp as Human));
    }
    else{
  ...
  public updateHuman(human: Human){
    const body = JSON.stringify(human);
    return this.http.put(this.url+"/humans/"+ human.id, body, {headers: this.headers, withCredentials: true}).pipe(map(resp => resp as Human));
  }
